...
def getWordScore(word, n):
    ...
    score_letters = 0
    for letter in word:
        score_letters = score_letters + SCRABBLE_LETTER_VALUES[letter]
    
    if n == len(word):
        return score_letters*len(word) + 50
    else:
        return score_letters*len(word)
...
def updateHand(hand, word):
    ...
    new_hand = hand.copy()
    for letter in word:
        num_times = new_hand.get(letter,0)
        if num_times > 0:
